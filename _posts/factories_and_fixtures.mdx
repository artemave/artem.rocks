...
I think we got it all wrong. It's not fixtures vs factories, it's fixtures *and* factories. In fact, it's factories and then fixtures — the order is important. Start off with factories, and then turn the common ones into fixtures.
...
Factories are slow because they exercise the model code to create test state. Fixtures, on the other hand, insert records straight into the test database using sql, bypassing model code. This, coupled with the fact that they are only inserted once before all tests, makes them a faster option[^1], but, as there are no model guardrails (e.g. validations), it's on the developer to manually make sure the data is valid. And that makes them hard.

So. Neither are superior. You might be better off using factories for some rare/edge cases (small performance impact across the board) and resort to fixtures otherwise. But the problem is that there is no easy way to turn one into another. Everything starts small and by the time it graduates into something more common, there are likely lots of interconnected models involved and replicating them manually in fixtures yaml is a hard work. No one wants to spend their day doing that.
...
This is no doubt fancy, but it may not be immediately obvious what's it got to do with factories.
...
All of the tests that are calling `create_user` should continue to pass. Except now they are using the fixture instead.
...
[^1]: Check out [my previous post](https://artem.rocks/posts/faster_rails_fixtures) to make fixtures even faster.
