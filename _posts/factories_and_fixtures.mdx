...
Factories are slow because they exercise the model code to create test state. Fixtures, on the other hand, insert records straight into the test database using sql, bypassing model code. They are faster at creating test state[^1], but, as there are not model guardrails (e.g. validations), it's on the developer to manually make sure the data is valid. Which makes them hard.

So. Neither are superior. You might be better off using factories for some rare/edge cases (small performance impact across the board) and resort to fixtures otherwise. But the problem is that there is no easy way to turn one into another. Everything starts small and by the time it graduates into something more common, there are likely lots of interconnected models and replicating them manually in fixtures yaml is a hard work. No one wants to spend their day doing that.
