...
Have you ever noticed that Rails always inserts test fixtures into the database, even if their definitions haven't changed? You have probably also noticed how Rails development server compiles browser assets just-in-time if needed. And, perhaps, you were wandering why wouldn't they do the same thing for fixtures? Or maybe you weren't, if you didn't work on a large enough codebase, where, I bet, you couldn't help but notice that it takes an awful lot of time.

If I haven't lost you already, read on - that's exactly the problem we're going to solve here.
...
We're going to patch some Rails code, so that it skips fixture inserts when all of the following conditions are met:

- fixture files haven't changed since the previous test run
- database schema hasn't changed since the previous test run
- fixtures digest is present (state of the fixtures from the previous run)

With this on, on a project with 195 fixture files, running an individual test (no parallelization) time dropped by over 30% (from 5 to 3.2 seconds on my machine). Running a test file (with parallelization) dropped by about 25% (from 11 to 8.2 seconds).

As a nice bonus, test logs are now free from thousands of lines of db inserts and other db setup gubbins.
